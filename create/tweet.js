...
var debunk = false;
const useTwemoji = false;
const unameRegex = /^@?[A-Za-z0-9_]+$/;
var interactions = {retweets: 0, quotes: 0, likes: 0};
var modals = {pfp: new bootstrap.Modal(document.getElementById("pfpModal")), img: new bootstrap.Modal(document.getElementById("imgModal")), date: new bootstrap.Modal(document.getElementById("dateModal"))};
...
function checkName(element, end) {
    ...
    if (length > 50) {
        $("#nameInfo").html(`<span style="color:red">${length}</span>/50 characters`);
    } else {
...
function checkUsername(element, end) {
    ...
    let length = (hasAt ? element.textContent.length - 1 : element.textContent.length);
    let info = "";
    ...
    if (unameRegex.test(element.textContent)) {
        ...
        info += ' <span style="color:red">Should only be numbers, letters, and _ characters.</span>';
    }
...
function charCount(element, end) {
    ...
    if (length > maxChars) {
        ...
        $("#charCounter").html(`<span style="color:red">${length}</span>/${maxChars} characters`);
    } else {
...
function editText(element) {
    $("#tweetText .simulatedLink").replaceWith(function () {return $(this).html();});
    charCount(element);
}
...
function checkDebunk() {
    if (!document.getElementById("debunkText").textContent && debunk) toggleDebunk();
}
...
const randomRetweet = [9, 69, 2, 531, 310, 2300, 1, 46, 32, 186, 17, 132, 35, 491, 32610, 64, 102, 203, 1009, 35, 186, 17234, 53420, 2504, 73512, 4, 5477, 4, 2503, 40333, 76, 1234, 6795, 1, 6, 166, 108, 140, 77, 969, 33, 3, 475, 9, 5577, 43, 6, 8, 87, 141, 315, 0, 10, 73, 18, 2419, 1314, 18969, 270, 2, 439, 9, 571, 1, 51];
const randomQuote = [21, 21, 2, 377, 214, 1988, 22, 24, 9, 79, 4, 25, 6, 66, 18928, 32, 30, 51, 5020, 4, 29, 1124, 22169, 834, 31132, 1, 1403, 0, 660, 11295, 14, 440, 792, 0, 3, 34, 21, 48, 17, 112, 10, 0, 94, 4, 212, 8, 1, 5, 18, 33, 124, 1, 72, 0, 4, 511, 175, 1213, 73, 0, 12, 2, 26, 2, 20];
const randomLike = [2021, 420, 21, 7214, 4602, 6589, 18, 1734, 348, 3442, 73, 624, 214, 1880, 74787, 422, 3414, 6990, 1705, 127, 3720, 79536, 110669, 15671, 325360, 233, 57539, 70, 20441, 196946, 619, 7211, 19455, 18, 275, 1698, 2632, 481, 492, 5287, 220, 24, 2899, 26, 20315, 165, 52, 39, 1024, 11002, 1440, 1, 959, 596, 1211, 18748, 7808, 84696, 5194, 51, 8266, 1377, 5656, 70, 284];
...
function generate() {
    ...
    $("#generateButton").html('<i class="fas fa-sync-alt spinner"></i>  Please wait...');
    ...
    setTimeout(function () {
        ...
        html2canvas(capture, {allowTaint: true, backgroundColor: bgColor, windowWidth: 640}).then(canvas => {
            ...
            canvas.toBlob(blob => {
                ...
                $("#generateButton").html('<i class="fas fa-download"></i>  Generate image');
